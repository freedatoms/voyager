...
bool WebPage::acceptNavigationRequest(QWebFrame *frame, const QNetworkRequest &request, NavigationType type)
{
    ...
    if ((type == QWebPage::NavigationTypeLinkClicked || type == QWebPage::NavigationTypeOther)
        && (m_keyboardModifiers & Qt::ControlModifier || m_keyboardModifiers & Qt::ShiftModifier
            || m_pressedButtons == Qt::MidButton)) {
        bool newWindow = (m_keyboardModifiers & Qt::ShiftModifier);
        WebView *webView;
        if (newWindow) {
            ...
            bool selectNewTab = (m_keyboardModifiers & Qt::ControlModifier);
...
QWebPage *WebPage::createWindow(QWebPage::WebWindowType type)
{
    ...
    if (m_openInNewTab){
    	if (m_keyboardModifiers & Qt::ControlModifier)
    		...
    		return mainWindow()->tabWidget()->makeNewTab(true)->page();
    }
    BrowserApplication::instance()->newMainWindow();
...
void WebPage::showftpClient(QUrl url){
 QFile file(QLatin1String(":/ftpclient.html"));
    if (!file.open(QIODevice::ReadOnly)) {
        qWarning() << "WebPage::handleUnsupportedContent" << "Unable to open ftpclient.html";
        return;
    }
    QString title = tr("Ftp Client");
    QString html = QString(QLatin1String(file.readAll()))
                        .arg(title)
                        .arg(url.toString());
    QBuffer imageBuffer;
    imageBuffer.open(QBuffer::ReadWrite);
    QIcon icon(QLatin1String(":/voyager.png"));
    QPixmap pixmap = icon.pixmap(QSize(128, 128));
    if (pixmap.save(&imageBuffer, "PNG")) {
        html.replace(QLatin1String("LOGO_IMAGE_BINARY_DATA_HERE"),
...
void WebPage::handleUnsupportedContent(QNetworkReply *reply)
{
    ...
    QBuffer imageBuffer;
    imageBuffer.open(QBuffer::ReadWrite);
    QIcon icon(QLatin1String(":dialog-warning.png"));
    QPixmap pixmap = icon.pixmap(QSize(128, 128));
    if (pixmap.save(&imageBuffer, "PNG")) {

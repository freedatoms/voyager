...
void SettingsDialog::loadFromSettings()
{
    ...
    QString defaultHome = QLatin1String("https://launchpad.net/voyager");
    ...
    bool alwaysPromptForFileName = settings.value(QLatin1String("alwaysPromptForFileName"), true).toBool();
 ...
 // Search Engines
    ...
    SEsel->addItems(m_urlsSE->stringList());
    //SEsel->setModel(m_urlsSE);
    settings.endGroup();
    // Appearance
...
void SettingsDialog::on_pb_add_clicked()
{
    BrowserMainWindow *mainWindow = BrowserApplication::instance()->mainWindow();
    QDialog dialog(mainWindow);
    dialog.setWindowFlags(Qt::Sheet);
    Ui::addSEdialog addsedialog;
    addsedialog.setupUi(&dialog);
    QString name;
    QString url;
    if (dialog.exec() == QDialog::Accepted) {
       name=addsedialog.le_name->text();
       url=addsedialog.le_url->text();
       if (!name.isEmpty() || !url.isEmpty()){
          QStringList urls=m_urlsSE->stringList();
          urls.append(name+QLatin1String("\\")+url);
          m_urlsSE->setStringList(urls);
          SEsel->clear();
          SEsel->addItems(urls);
      }
    }
...
void SettingsDialog::on_SEsel_doubleClicked(const QModelIndex & index){
    BrowserMainWindow *mainWindow = BrowserApplication::instance()->mainWindow();
    QDialog dialog(mainWindow);
    dialog.setWindowFlags(Qt::Sheet);
    Ui::addSEdialog addsedialog;
    ...
    QString name;
    QString url;
    addsedialog.le_name->setText(m_urlsSE->stringList().at(index.row()).split(QLatin1String("\\")).at(0));
    addsedialog.le_url->setText(m_urlsSE->stringList().at(index.row()).split(QLatin1String("\\")).at(1));
    if (dialog.exec() == QDialog::Accepted) {
       name=addsedialog.le_name->text();
       url=addsedialog.le_url->text();
       if (!name.isEmpty() || !url.isEmpty()){
          QStringList urls=m_urlsSE->stringList();
          urls.replace(index.row(),name+QLatin1String("\\")+url);
          m_urlsSE->setStringList(urls);
          SEsel->clear();
          SEsel->addItems(urls);
      }
...
void SettingsDialog::on_pb_restore_clicked()
{
QStringList url;
...
void SettingsDialog::on_pb_remove_clicked()
{
    QStringList list = m_urlsSE->stringList();
    list.removeAt(SEsel->currentRow());
    m_urlsSE->setStringList(list);
    SEsel->takeItem(SEsel->currentRow());
}

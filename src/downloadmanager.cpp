...
BrowserMainWindow *DownloadManager::mainWindow()
{
...
QStringList DownloadManager::getFiles(){
    QStringList list;
    for (int i=0;i<m_downloads.count();i++){
    list.append(m_downloads.at(i)->fileNameLabel->text());
    }
    return list;
}
DownloadItem* DownloadManager::getItem(QString file){
    QStringList list=getFiles();
    return m_downloads.at(list.indexOf(file));
}
QStringList DownloadManager::getInfos(){
    QStringList list;
    for (int i=0;i<m_downloads.count();i++){
    list.append(m_downloads.at(i)->downloadInfoLabel->text());
    }
    return list;
}
QStringList DownloadManager::getLocations(){
    QStringList list;
    for (int i=0;i<m_downloads.count();i++){
        list.append(m_downloads.at(i)->m_url.toString());
    }
    return list;
}

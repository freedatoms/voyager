...
{
    qRegisterMetaType<FtpWindow>();

    QCoreApplication::setOrganizationDomain(QLatin1String("launchpad.net"));
    QCoreApplication::setApplicationName(QLatin1String("Voyager"));
    QString version = QLatin1String("0.0.97");
...
void BrowserApplication::postLaunch()
{
    ...
    setWindowIcon(QIcon(QLatin1String(":/voyager.png")));
...
{
    {
        ...
        if (settings.value(QLatin1String("restoring"), false).toBool()) {
            QMessageBox::information(0, tr("Restore failed"),
                tr("The saved session will not be restored because Voyager crashed while trying to restore this session."));
            return false;
        }
...
QIcon BrowserApplication::icon(const QUrl &url)
{
    ...
    if (icon.isNull()) {
        QPixmap pixmap = QPixmap(QLatin1String(":/voyager.png"));

    ...
    , m_tabBar(new TabBar(this))
{
    ...
    // Actions
    m_newTabAction = new QAction(QIcon(QLatin1String(":/tab-new.png")),tr("New &Tab"), this);
    m_newTabAction->setShortcuts(QKeySequence::AddTab);
    connect(m_newTabAction, SIGNAL(triggered()), this, SLOT(newTab()));

    m_closeTabAction = new QAction(QIcon(QLatin1String(":/tab-close.png")),tr("&Close Tab"), this);
    m_closeTabAction->setShortcuts(QKeySequence::Close);
    connect(m_closeTabAction, SIGNAL(triggered()), this, SLOT(closeTab()));

#if QT_VERSION < 0x040500
    m_newTabAction->setIcon(QIcon(QLatin1String(":/tab-new.png")));
    m_newTabAction->setIconVisibleInMenu(false);

    m_closeTabAction->setIcon(QIcon(QLatin1String(":/tab-close.png")));
    m_closeTabAction->setIconVisibleInMenu(false);
#endif

    m_nextTabAction = new QAction(QIcon(QLatin1String(":/tab-next.png")),tr("Show Next Tab"), this);
    ...
    m_previousTabAction = new QAction(QIcon(QLatin1String(":/tab-previous.png")),tr("Show Previous Tab"), this);
    shortcuts.clear();
    ...
    m_tabBar->setSelectionBehaviorOnRemove(QTabBar::SelectPreviousTab);
      ...
      m_expo=new QAction(QIcon(QLatin1String(":/expo.png")),tr("Expo"),this);
      connect(m_expo, SIGNAL(triggered()), this, SLOT(expo()));
      QToolButton *ExpoButton = new QToolButton(this);
      ExpoButton->setDefaultAction(m_expo);
      ExpoButton->setAutoRaise(true);
      ExpoButton->setToolButtonStyle(Qt::ToolButtonIconOnly);
      setCornerWidget(ExpoButton, Qt::TopLeftCorner);
...
{
	expoc->click();
	expoc=new QPushButton(BrowserApplication::instance()->mainWindow());
	BrowserApplication::instance()->mainWindow()->layout->addWidget(expoc);
	expoc->setSizePolicy(QSizePolicy::Ignored,QSizePolicy::Ignored);
	QGridLayout *gridLayout = new QGridLayout(expoc);
	gridLayout->setAlignment(Qt::AlignHCenter);
	gridLayout->setSpacing(6);
	expoc->setLayout(gridLayout);
	expoc->setStyleSheet(QLatin1String("background:#888;border:0;"));
	connect(expoc,SIGNAL(clicked()),this,SLOT(expo_close()));
	int k=1;
	if ((count()>1)&&(count()<5)) k=2;
	if ((count()>4)&&(count()<10)) k=3;
	if ((count()>9)&&(count()<17)) k=4;
	if ((count()>16)&&(count()<26)) k=5;
	if ((count()>25)&&(count()<37)) k=6;
	if ((count()>36)&&(count()<50)) k=7;
	if ((count()>49)&&(count()<65)) k=8;
	if ((count()>64)&&(count()<82)) k=9;
	if ((count()>81)&&(count()<101)) k=10;
	if (count()>100) k=15;
	for (int i = 0; i < count(); ++i) {
        ExpoItem *item=new ExpoItem(QPixmap::grabWidget(webView(i)),tabText(i),(size()/k)*0.9,i,expoc);
        gridLayout->addWidget(item, i/k,i%k,Qt::AlignLeft);
    }
	this->setVisible(false);
	expoc->show();
}

void TabWidget::expo_close(){
    expoc->close();
